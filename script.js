window.onload = function() {
    // 1. Função de Distribuição de Temperatura (O coração do cálculo)
    ...
    // 2. Lógica das Abas (Corrigida para alternar texto e renderizar fórmulas)
    const configurarAba = (idBtn, idConteudo, texto) => {
        ...
        if (btn && conteudo) {
            btn.onclick = function() {
                ...
                // Força o MathJax a ler os cálculos quando a aba abre
    ...
    // 3. Gráfico 3D (Plotly) - Resolve o quadro branco da imagem 1
    ...
    if (cv2d) {
        ...
        // Desenha o diferencial dA
        ctx.fillStyle = '#e74c3c';
        ctx.fillRect(cx + 2 * escala, cy - 2 * escala, 15, 15);
    }

    // 5. Gráfico de Corte (Perfil Gaussiano) - Resolve a imagem 3
    ...
    if (cvCorte) {
        ...
        for(let x = -6; x <= 6; x += 0.1) {
            ...
            let posY = cy - (getTempAt(x, 0) * 0.8); // Escala para caber no canvas
